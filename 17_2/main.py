...
def border_dist(pos: Pos, d: Direction, height: int, width: int, max_steps: int) -> int:
    ...
    if d == N:
        ...
        return min(height - y - 1, max_steps)
    elif d == W:
        return min(x, max_steps)
    else: # d == E
        return min(width - x - 1, max_steps)
...
def outgoing_edges(heat_loss_map: List[List[int]], node: Node) -> List[Edge]:
    ...
    steps = border_dist(p, d, height, width, 10)
    weight = 0
    turn_dirs = (E, W) if d in [N, S] else (N, S)
    for l in range(1, min(4, steps+1)):
        new_pos = move(p, d, l)
        weight += heat_loss_map[new_pos[1]][new_pos[0]]
    for l in range(4, steps+1):
        new_pos = move(p, d, l)
        weight += heat_loss_map[new_pos[1]][new_pos[0]]
        yield ((p, d), (new_pos, turn_dirs[0]), weight)
        yield ((p, d), (new_pos, turn_dirs[1]), weight)
